"""Tests for mazepa_to search engine plugin."""
...
from plugins.search.mazepa_to import (
    FORUM_MAP,
    Config,
    ConfigJson,
    LoginPayload,
...
class TestMazepaHTMLParser:
    ...
    def test_parse_ukrainian_size_units(self) -> None:
        ...
        """
        # Ukrainian size string -> expected bytes
        size_tests = [
            ("500 МБ", (500 * 1024**2)),
            ("2.3 ГБ", int(2.3 * 1024**3)),
            ("750 КБ", (750 * 1024)),
            ("1 ТБ", (1 * 1024**4)),
        ]
        for size_str, expected_bytes in size_tests:
...
class TestIntegration:
    ...
    def test_config_round_trip(self) -> None:
        original = Config(
            credentials=LoginPayload(login_username="testuser", login_password="testpass"),  # nosec B106
            cache_login_cookies=False,
        )
